...
public class ComparativeStringAnalyser {
	...
	protected static Comparative decodeComparativeForOuter(String compValue) {
		boolean containsIn = StringUtil.contains(compValue, " in");
		...
		if (!containsIn) {
			...
			String valueTypeStr=StringUtil.substring(compValue, index + size);
			...
			if (null != type &&null !=value) {
				if ("i".equals(type.trim())) {
					comparative = new Comparative(compEnum,
			...
			if (null != inValues && null!=type) {
				...
				String[] values=StringUtil.split(getBetween(inValues, "(", ")"),",");
				if ("i".equals(type.trim())) {
					for(String value:values){
						Comparative temp = new Comparative(Comparative.Equivalent,
							Integer.valueOf(value.trim()));
					...
					for(String value:values){
						...
						try{
							temp = new Comparative(Comparative.Equivalent,
									sdf.parse(value.trim()));
						}catch (ParseException e) {
							throw new RuntimeException(
									"only support 'yyyy-MM-dd',now date string is:"
