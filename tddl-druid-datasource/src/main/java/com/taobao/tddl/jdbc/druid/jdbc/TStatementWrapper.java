...
public class TStatementWrapper implements Statement {
	...
	protected void recordReadTimes() throws SQLException {
		DruidDbStatusEnum status = datasourceWrapper.connectionProperties.dbStatus;
		if (status != DruidDbStatusEnum.R_STATUS && status != DruidDbStatusEnum.RW_STATUS) {
	...
	protected void recordWriteTimes() throws SQLException {
		DruidDbStatusEnum status = datasourceWrapper.connectionProperties.dbStatus;
		if (status != DruidDbStatusEnum.W_STATUS && status != DruidDbStatusEnum.RW_STATUS) {
